...
namespace Knives
{
    ...
    {
        private static Action<Gun> OnShotProjectile = (x) => { };
        ...
        public static void ShootSingleProjectile(Action<Gun, ProjectileModule, ProjectileData, GameObject> orig, Gun self, ProjectileModule mod, ProjectileData overrideProjectileData, GameObject overrideBulletObject)
        {
            if (self.encounterTrackable.EncounterGuid == "Ski's Ratapult")
            {
                self.StartCoroutine(WaitForProjectile(orig, self, mod, overrideProjectileData, overrideBulletObject));
            }
            else
            {
                orig(self, mod, overrideProjectileData, overrideBulletObject);
            }
        }

        private static IEnumerator WaitForProjectile(Action<Gun, ProjectileModule, ProjectileData, GameObject> orig, Gun self, ProjectileModule mod, ProjectileData overrideProjectileData = null, GameObject overrideBulletObject = null)
        {
            float fireDelay = .3f;

            if (self.CurrentOwner is PlayerController player)
            {
                float fireMultiplier = player.stats.GetStatValue(PlayerStats.StatType.RateOfFire);

                fireDelay *= 1f / fireMultiplier;
            }
