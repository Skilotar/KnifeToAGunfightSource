            ...
            if (Enemy != null && Enemy.aiActor != null)
            {
                ...
                if (Enemy && Enemy.aiActor && Enemy.aiActor.IsNormalEnemy && state.LootedByBaba == false)
                {
                    ...
                    { 
                        ...
                        LootEngine.SpawnItem(PickupObjectDatabase.GetById(PickupObjectDatabase.GetByName("Tiny Ammo Pack").PickupObjectId).gameObject, Enemy.sprite.WorldCenter, vector * -1, 5f, false, false, false);
